...
export default function Login() {
  ...
  const [accessToken, setAccessToken] = useState<string>(""); // To store access token
  ...
  const getAccessToken = async () => {
    const data = new URLSearchParams({
      username,
      password,
      client_id: "fb9aa79b-3ccd-401b-997d-3d505eec99ea",
    ...
    try {
      const response = await axios.post(
        "https://login.microsoftonline.com/c5c2b8e1-ef9b-46bc-ab47-6a4d1c55e80f/oauth2/v2.0/token",
        data,
        {
          headers: {
      ...
      setAccessToken(accessToken); // Save token in state
      setMessage("Access Token Obtained");
    } catch (error) {
  ...
  // Function to use access token and get user info
  const getUserInfo = async () => {
    ...
    try {
      const response = await axios.get("https://graph.microsoft.com/beta/me", {
        headers: {
      ...
      setUserInfo(JSON.stringify(userData, null, 2)); // Set user info to display on screen
      setMessage("User info fetched successfully.");
    } catch (error) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response.status === 201) {
        ...
        await getAccessToken(); // Get access token after successful login
        await getUserInfo(); // Fetch user info with the obtained token
