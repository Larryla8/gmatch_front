...
export default function Login() {
  ...
  const getAccessToken = async (username: string, password: string) => {
    ...
    const config = {
      method: "post",
      url: "https://login.microsoftonline.com/c5c2b8e1-ef9b-46bc-ab47-6a4d1c55e80f/oauth2/v2.0/token",
      headers: {
    ...
    try {
      ...
      setMessage("Access token received. Check console for details.");
    } catch (error) {
  ...
  const handleLogin = async () => {
    ...
    try {
      // Call the API to get the access token using the username and password
